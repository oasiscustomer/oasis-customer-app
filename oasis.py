...
# oasis.py - 모든 오류 제거 + 중복 등록 방지 + 전화번호 포맷 적용 + KST 적용 최종본
...
if matched:
    ...
    if new_plate and new_phone:
        ...
        already_visited_today = existing_customer and today in str(existing_customer.get("방문기록", ""))
        ...
        if existing_customer:
            ...
            if already_visited_today:
                st.info("📌 오늘도 이미 방문한 고객입니다. 추가 입력할까요?")
            else:
                st.info("📌 오늘 방문 기록은 아직 없습니다. 등록할까요?")

            confirm_new = st.radio("입력 확인", ["Y", "N"], key="new_confirm")
            if st.button("✅ 등록 확인"):
                if confirm_new == "Y":
                    try:
                        row_idx = next((i + 2 for i, r in enumerate(records) if r.get("차량번호") == new_plate), None)
                        ...
                        worksheet.update(f"E{row_idx}", [[new_count]])
                        worksheet.update(f"F{row_idx}", [[new_log]])
                        st.success("✅ 기존 고객의 방문 정보가 추가되었습니다.")
                    except Exception as e:
                        st.error(f"❌ 업데이트 실패: {e}")
                else:
                    st.stop()
