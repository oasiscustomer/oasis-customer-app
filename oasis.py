...
if matched:
    ...
    # ✅ 선택한 번호가 기존 고객 리스트에 있는지 확인
    all_records = worksheet.get_all_records()
    existing_customer = next((r for r in all_records if r["차량번호"] == selected_plate), None)

    if existing_customer:
        visit_log = existing_customer.get("방문기록", "")
        row_idx = next((i + 2 for i, r in enumerate(all_records) if r["차량번호"] == selected_plate), None)
        ...
        st.info("🆕 선택한 차량번호는 등록되지 않았습니다. 아래에 고객 정보를 입력해 주세요.")
        with st.form("new_customer_form"):
            ...
            if st.form_submit_button("📥 신규 고객 등록"):
                ...
                try:
                    new_row = [selected_plate, formatted_phone, today, today, 1, f"{now_str} (1)"]
    ...
    with st.form("register_form"):
        ...
        if register_submit and new_plate_input and new_phone_input:
            all_records = worksheet.get_all_records()
            already_exists = any(r["차량번호"] == new_plate_input for r in all_records)
            formatted_phone = format_phone_number(new_phone_input)
