...
# oasis.py - 최종 안정화 버전 (KST 적용 + 인증 스코프 + 문법 오류 수정 포함)
    ...
    if customer:
        ...
        if st.session_state.confirm_update and st.session_state.confirmation_mode:
            ...
            if row_idx:
                ...
                try:
                    worksheet.update(f"D{row_idx}", [[today]])        # 최근 방문일
                    worksheet.update(f"E{row_idx}", [[new_count]])    # 총 방문 횟수
                    worksheet.update(f"F{row_idx}", [[new_log]])      # 방문기록
    ...
    if new_plate and new_phone:
        ...
        if already_exists_today:
            ...
            if st.button("✅ 신규 확인"):
                if confirm_new == "Y":
                    try:
                        new_row = [new_plate, new_phone, today, today, 1, f"{now_str} (1)"]
