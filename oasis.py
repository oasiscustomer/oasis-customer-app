...
Streamlit Cloud + Google Colab 호환 고객관리 시스템 (중복 차량 중복 입력 방지 및 예외 처리 포함)
"""
...
if search_button and plate_input:
    ...
    matched = [
        r for r in records
        if isinstance(r.get("차량번호"), str) and (
...
if st.session_state.search_done:
    ...
    if customer:
        ...
        if row_index:
            ...
            if date_only in customer.get("방문기록", ""):
                st.info("📌 오늘 이미 입력된 기록이 있습니다. 추가로 입력할까요?")
                confirm_add = st.radio("입력 확인", ["Y", "N"], key="confirm_add")
                if confirm_add == "N":
                    st.stop()
        ...
        if st.button("📥 신규 고객 등록") and full_plate and raw_phone:
            existing = [r for r in worksheet.get_all_records() if r.get("차량번호") == full_plate]
            if existing:
                st.warning("⚠️ 이미 등록된 차량번호입니다. 중복 등록을 방지하세요.")
                st.stop()
