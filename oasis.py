...
if matched:
    ...
    # ✅ 입력값과 선택값이 정확히 일치할 때만 기존 고객 처리
    if st.session_state.input_plate_value == selected_plate:
        ...
        st.warning("🚫 입력한 번호와 선택한 차량번호가 다릅니다. 신규 등록이 필요합니다.")
        st.session_state.matched_customers = []  # 신규 등록으로 유도

# ✅ 신규 고객 등록
if not st.session_state.matched_customers:
    ...
    with st.form("register_form"):
        ...
        if register_submit and new_plate_input and new_phone_input:
            all_records = worksheet.get_all_records()
            existing_customer = next((r for r in all_records if r.get("차량번호") == new_plate_input), None)
            formatted_phone = format_phone_number(new_phone_input)

            if existing_customer:
                st.warning("🚨 이미 등록된 고객입니다.")
                confirm = st.radio("오늘 추가 방문으로 기록할까요?", ["Y", "N"], key="exist_confirm")
                if st.button("✅ 기존 고객 등록 확인"):
                    if confirm == "Y":
                        row_idx = next((i + 2 for i, r in enumerate(all_records) if r["차량번호"] == new_plate_input), None)
                        try:
