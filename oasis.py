...
(인증 오류 방지 + 신규 고객 입력 필드 유지 + 중복 차량 선택 에러 수정)
"""
...
if "matched_result" not in st.session_state:
    st.session_state.matched_result = None
...
if search_button and plate_input:
    ...
    st.session_state.selected_plate = None  # 초기화
...
if st.session_state.search_done:
    ...
    if matched and len(matched) == 1:
        ...
        if selected and st.session_state.selected_plate != selected:
            st.session_state.selected_plate = selected
            st.session_state.search_done = True
            st.session_state.matched_result = [r for r in matched if r['차량번호'] == selected]
