...
if matched:
    ...
    if customer:
        ...
        if today in visit_log:
            ...
            if st.button("✅ 확인"):
                if choice == "Y":
                    row_idx = next((i + 2 for i, r in enumerate(records) if r['차량번호'] == customer['차량번호']), None)
                    if row_idx:
                        try:
    ...
    with st.form("register_form"):
        ...
        if register_submit and new_plate_input and new_phone_input:
            records = worksheet.get_all_records()
            existing_customer = next((r for r in records if r.get("차량번호") == new_plate_input), None)
            ...
            if existing_customer:
                ...
                if st.button("✅ 기존 고객 등록 확인"):
                    if confirm == "Y":
                        try:
                            row_idx = next((i + 2 for i, r in enumerate(records) if r["차량번호"] == new_plate_input), None)
