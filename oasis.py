...
def install_packages():
    required_packages = ["gspread", "oauth2client", "pytesseract", "opencv-python", "streamlit"]
...
# 🚨 Streamlit Cloud에서는 `pip install`이 자동 실행되지 않으므로, 로컬 환경 및 Colab에서만 실행
if "google.colab" in sys.modules or "streamlit" not in sys.modules:
    install_packages()
...
print(f"✅ JSON 키 파일 업로드 성공: {json_key}")